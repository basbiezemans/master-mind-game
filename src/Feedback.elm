...
import Utils exposing (unequal, elem, delete, zip)

type alias Feedback =
    { correct : Int -- Number of digits that are guessed correctly.
    , present : Int -- Number of digits that are present in the code
...
makeFeedback secret guess =
    let
        pairs = listOfPairs secret guess
    in
...
numPresent pairs =
    let
        (secret, digits) = List.unzip pairs
    in
...
toString feedback =
    let
        correct = String.repeat feedback.correct "●"
        present = String.repeat feedback.present "○"
    in
    correct ++ present
