...
feedbackInclusionTest =
    let
        secret =
            Secret.fromList [ 6, 2, 4, 3 ]

        guess =
            Guess.fromList [ 1, 2, 3, 4 ]

        -- 2, 3 and 4 are included, 2 also has the correct position
        answer =
            { correct = 1
            , present = 2
            }
...
feedbackDuplicatesTest2 =
    let
        secret =
            Secret.fromList [ 5, 2, 5, 6 ]

        guess =
            Guess.fromList [ 2, 2, 4, 4 ]
...
feedbackDuplicatesTest3 =
    let
        secret =
            Secret.fromList [ 6, 4, 4, 3 ]

        guess =
            Guess.fromList [ 4, 1, 2, 4 ]
...
feedbackDuplicatesTest4 =
    let
        secret =
            Secret.fromList [ 6, 4, 2, 3 ]

        guess =
            Guess.fromList [ 2, 2, 5, 2 ]
...
feedbackDuplicatesTest5 =
    let
        secret =
            Secret.fromList [ 6, 1, 6, 3 ]

        guess =
            Guess.fromList [ 1, 1, 3, 6 ]
